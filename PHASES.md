...
> - Graceful degradation: typed `HTTPError` + `ChatUnavailable` helper; TUI banner + completions-only skip wired 2026-08-21 (review follow-up, owner-approved), version probe in `versions.go`.
...
- [x] Graceful degradation: if chats endpoints fail, fall back to completions-only mode (models + stream, no session sync) with a clear user notice (D5). *(Implemented 2026-08-21 (adversarial-review follow-up, owner-approved admin action): TUI banner + chat-list skip when chats fail or the version is outside the pinned window; `GET /api/version` probe + `internal/openwebui/versions.go`; CI smoke `cmd/smoke` + `.github/workflows/ci.yml`.)*
